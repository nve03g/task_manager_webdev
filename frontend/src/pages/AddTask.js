...
const AddTask = () => {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      try {
        const response = await fetch(`https://localhost:443/projects/${projectId}/check-admin`, {
          headers: {
        ...
        if (response.ok && data.isAdmin) {
          ...
          throw new Error(data.error || 'You do not have permission to add tasks to this project.');
        }
      } catch (err) {
